...
test('CaptureService.processCapture：成功流程（VLM 200 + JSON 解析）', async () => {
  ...
  assert.ok(result.image.size > 0); // 壓縮後有輸出（極小圖片可能微幅增大）
});
...
test('CaptureService.processCapture：圖片壓縮後應有輸出檔案', async () => {
  const mockVLM = { recognize: async () => ({ success: true, content: '{"name":"Test","price":100}', raw: {}, attempts: 1, latencyMs: 1 }) };
  const service = new CaptureService({ vlmClient: mockVLM });
...
test('CaptureService.processCapture：接受所有支援的 MIME 類型', async () => {
  ...
  for (const mimetype of ['image/jpeg', 'image/png', 'image/webp']) {
    ...
    assert.ok(result.image.size > 0, `${mimetype} 應產生輸出`);
    if (result.image && result.image.path) fs.unlinkSync(result.image.path);
  }
});
...
test('createUploadMiddleware：應拒絕不支援的 MIME', async () => {
  const { createUploadMiddleware, ALLOWED_MIME_TYPES } = require('../../src/middleware/upload');
  const middleware = createUploadMiddleware({ env: {}, maxSizeMB: 5 });

  // 建立 mock request/response
  const req = { file: null };
  const res = {};
  ...
  // 測試不支援的 MIME（直接呼叫 fileFilter 內部邏輯）
  ...
  fileFilter(req, mockFile, (err, accept) => {
    assert.ok(err, '應產生錯誤');
    assert.equal(err.code, 'UNSUPPORTED_MIME');
    assert.equal(err.status, 415);
  });
});

test('createUploadMiddleware：應接受支援的 MIME', async () => {
  const { createUploadMiddleware, ALLOWED_MIME_TYPES } = require('../../src/middleware/upload');
  ...
  fileFilter({}, mockFile, (err, accept) => {
    assert.equal(err, null, '不應有錯誤');
    assert.equal(accept, true, '應接受');
  });
});
