...
test('ItemRepository.findByFingerprint：依 fingerprint 查詢商品（依時間倒序）', () => {
  // 新增多筆
  itemRepo.create({ fingerprint: 'fp-bob', name: 'Milk', price: 95, currency: 'TWD' });
  itemRepo.create({ fingerprint: 'fp-bob', name: 'Bread', price: 65, currency: 'TWD' });
  itemRepo.create({ fingerprint: 'fp-bob', name: 'Cheese', price: 120, currency: 'TWD' });

  const items = itemRepo.findByFingerprint('fp-bob');
...
test('ItemRepository.findByFingerprint：分頁測試', () => {
  const items = itemRepo.findByFingerprint('fp-bob', { limit: 2, offset: 0 });
  assert.equal(items.length, 2);
  assert.equal(items[0].name, 'Cheese');
  assert.equal(items[1].name, 'Bread');

  const items2 = itemRepo.findByFingerprint('fp-bob', { limit: 2, offset: 2 });
