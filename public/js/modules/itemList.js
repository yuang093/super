...
async function loadItems() {
  ...
  try {
    const response = await fetch(`/api/items?fingerprint=${encodeURIComponent(fingerprint)}&limit=100`, {
...
async function deleteItem(id) {
  ...
  try {
    const response = await fetch(`/api/items/${id}?fingerprint=${encodeURIComponent(fingerprint)}`, {
      method: 'DELETE',
    });
    const data = await response.json();
